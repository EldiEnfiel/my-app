...
`THREE_D_EARTH_GITHUB_REPO` は `owner/repo` 形式です。省略時はローカル repo の `origin` から自動推定します。`start-remote-3d-earth.bat` は EC2 起動後に current public IP を見て、GitHub の `DEPLOY_HOST` と `DEPLOY_KNOWN_HOSTS` を自動更新します。同期だけ止めたい場合は `-SkipGitHubDeploySync` を付けてください。
